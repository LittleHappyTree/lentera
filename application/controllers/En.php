...
class En extends CI_Controller {
	...
	function booking($value="",$id=""){
		...
		if ($value=='process') {
			...
			$data['page'] = 'booking/expired';
			$this->load->view('frame',$data);
		} elseif ($value=='confsuccess') {
			$data['checkid'] = $id;
			$data['page'] = 'booking/confirm_success';
			$this->load->view('frame',$data);
	...
	function send($id=''){
		...
		$count = $this->models->countrows($sql,array($id));
		$token = $this->models->getdata($sql,array($id),'token');
		...
		if ($count > 0) {
			...
			$message = '<!DOCTYPE html>
						...
						<span style="display:none">ORDER CONFIRMATION Hi <span style="color: #f82249;"><b>'.$nama.'</b></span>, thank you for booking on Lentera Travel. There is one more step to complete your booking, please choose one of two confirm method below.<br>This link below only valid for next 2 hours.</span>
										...
										<tr>
											<td>
												<div class="text" style="padding: 0 4em; text-align: center;">
													...
													<p>Hi <span style="color: #f82249;"><b>'.$nama.'</b></span>, thank you for booking on Lentera Travel. There is one more step to complete your booking, please choose one of two confirm method below.<br>This link below only valid for next 2 hours.</p>
													<p><a href="http://littlehappytree.com/demo/lentera/confirmation/order/web/'.$token.'/'.$id.'" style="  background: #f82249; border: 0; padding: 6px 22px; color: #fff; transition: 0.4s; border-radius: 50px;" class="btn btn-primary">Confirm by Website</a></p>
													<p>or</p>
													<p><a href="http://littlehappytree.com/demo/lentera/confirmation/order/whatsapp/'.$token.'/'.$id.'" style="  background: #f82249; border: 0; padding: 6px 22px; color: #fff; transition: 0.4s; border-radius: 50px;" class="btn btn-primary">Confirm by Whatsapp</a></p>
												</div>
											</td>
										</tr>
									</table>
